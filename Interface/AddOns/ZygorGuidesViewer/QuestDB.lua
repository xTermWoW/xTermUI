...
function ZGV.QuestDB:Init()
	...
	for gname,guidequests in pairs(ZGV.Quest_Cache) do
		for index,questdata in pairs(guidequests) do if type(questdata)=="table" then
			...
			-- only if conditions result on the other hand can change at any time, so set enviroment, and leave it to be queried properly
			if questdata.cond_if then setfenv(questdata.cond_if,ZGV.Parser.ConditionEnv) end
		end end
	end
...
hooksecurefunc("QuestMapFrame_ShowQuestDetails",function(questID) ZGV.QuestDB:SetQuestForButton(questID) end)
...
function ZGV.QuestDB:SetQuestForButton(questID)
