...
function Pointer:Startup()
	...
	if ZGV.DEV then
		...
		self.OverlayFrame.ZygorCoordsDEV = ZGV.ChainCall(worldMap_TargetFrame_UI:CreateFontString(nil,"ARTWORK","GameFontHighlight")) :SetPoint("BOTTOMLEFT",10,35) :SetPoint("BOTTOMRIGHT") :SetJustifyH("LEFT") :Hide() .__END 
...
function Pointer.Overlay_OnUpdate(frame,but,...)
	...
	if frame.ZygorCoordsDEV and frame.ZygorCoordsDEV:IsVisible() then
		...
		local s=("Cursor: %s|cff888888##|cffffffff%d (%s/%d) %.2f,%.2f (map type: %s%s)\n"):format(ZGV.GetMapNameByID(mm),mm, mmf and mmf[1] or ZGV.GetMapNameByID(mm),mmf and mmf[2] or 0, mx*100,my*100,maptype[ZGV.GetMapInfo(mm).mapType] or "type "..ZGV.GetMapInfo(mm).mapType, (LibRover.data.ZoneMeta[mm] and LibRover.data.ZoneMeta[mm].hostile and ", hostile" or ""))
		s = s .. ("Player: %s|cff888888##|cffffffff%d (%s/%d) %.2f,%.2f\n"):format(ZGV.GetMapNameByID(pm),pm, pmf and pmf[1] or ZGV.GetMapNameByID(pm),pmf and pmf[2] or 0, px*100,py*100)
		if mm~=pm and tx>0 and ty>0 then s = s .. ("Player on %s|cff888888##|cffffffff%d: %.2f,%.2f\n"):format(ZGV.GetMapNameByID(mm),mm,tx,ty) end
		s = s .. ("Distance: %.2f yd"):format(HBD:GetZoneDistance(pm,px,py, mm,mx,my) or 0)
		frame.ZygorCoordsDEV:SetText(s)
